    ...
    public class OrderPlaceService : IOrderPlaceService
    {
        ...
        private async Task OrderPlaceForFtpZipFile(ClientExternalOrderFTPSetupModel ftp, FtpCredentailsModel sourceFtpCredential, CompanyModel company, CompanyGeneralSettingModel companyGeneralSetting, FtpCredentailsModel kowToMateFtpCredentails, FileServerModel fileServer, FtpConfig ftpConfig, List<string> zipImagePaths)
        {
            if (zipImagePaths != null && zipImagePaths.Any())
            {
                ...
                {
                    //string zipPath = @"F:\Rakib\KowToMate\ConsoleApps\HtpOrderPlaceApp\bin\Debug\net8.0\TemporaryStore\ARC\TO KOW\OUTDOORS 1 Test By KOW.zip";
                    //string extractPath = @"F:\Rakib\KowToMate\ConsoleApps\HtpOrderPlaceApp\bin\\Debug\net8.0\TemporaryStore\ARC\TO KOW\Try";

                    // Ensure the extraction directory exists
                    


                    Console.WriteLine("Unzip completed with skips!");
                    ...
                    try
                    {
                        ...
                        using (var sourceClientFtp = new AsyncFtpClient(sourceFtpCredential.Host, sourceFtpCredential.UserName, sourceFtpCredential.Password, sourceFtpCredential.Port ?? 0, ftpConfig))
                        {
                            ...
                            //Extract Zip File
                            //TODO:Rakib for Arc client doing this urgent basis
                            if(companyGeneralSetting.CompanyId == 1192)
                            {
                                ZipFileExtractOnAppPublishedLocationAndSanitize(sourcePath, destination,companyGeneralSetting.CompanyId);
                            }
                            else
                            {
                                ...
                                using (ZipArchive archive = ZipFile.OpenRead(sourcePath))
                                {
                                    foreach (ZipArchiveEntry entry in archive.Entries)
                                    {
                                        ...
                                        if (entry.FullName.StartsWith("__MACOSX") || entry.FullName.EndsWith(".DS_Store"))
                                        {
                                            continue;
                                        }
        ...
        private void ZipFileExtractOnAppPublishedLocationAndSanitize(string sourcePath, string destination,int companyId)
        {
            if (!Directory.Exists(destination))
            {
                using (ZipArchive archive = ZipFile.OpenRead(sourcePath))
            ...
            FileServerModel fileServer, string orderDirectory, CompanyGeneralSettingModel companyGeneralSetting, bool isMoveSingleFile = false, bool isLocalFile = false)
        {
            ...
            try
            {
                ...
                if (orderSaveResponse.IsSuccess)
                {
                            ...
                            while (!uploadSuccessful && retryCount < 3)
                            {
                                try
                                {
                                    ...
                                    using (var destinationClient = new AsyncFtpClient(destinationFtpCredentails.Host, destinationFtpCredentails.UserName, destinationFtpCredentails.Password, destinationFtpCredentails.Port ?? 0, ftpConfig))
                                    {
                                        ...
                                        //TODO: This code doesnot work. Because send order Item id but in sp try to delete with objid
                                        else
                                        {
                    ...
                    if (!isLocalFile)
                    {
                        await MoveBatchAfterOrderPlace(sourceFtpCredential, orderDirectory, companyGeneralSetting, isMoveSingleFile, orderDirectoryPaths);
                    }
                }
            ...
            await SendOrderPlaceMailToClient(company, companyGeneralSetting, orderSaveResponse);
        ...
        private async Task SendOrderPlaceMailToClient(CompanyModel company, CompanyGeneralSettingModel companyGeneralSetting, Response<ClientOrderModel> orderSaveResponse)
        {
        ...
        private async Task MoveBatchAfterOrderPlace(FtpCredentailsModel sourceFtpCredential, string orderDirectory, CompanyGeneralSettingModel companyGeneralSetting, bool isMoveSingleFile, List<string> orderDirectoryPaths)
        {
