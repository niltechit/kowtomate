...
var driveManager = {
    pageInitialize: function () {
        ...
        driveManager.loadFencyTree();
        driveManager.initializeImageSelectors();
        driveManager.initializeImagePreview();
    },
    loadFencyTree: function () {
        $("#dashboard-attribute").css("display", "none");
        $("#fancytree").fancytree({
            source: { url: "/images/GetTreeData?path=", cache: false },
            lazyLoad: function (event, data) {
                var node = data.node;
                // Issue an Ajax request to load child nodes
                data.result = {
                    url: "/images/GetTreeData?path=" + node.key,
                    data: { key: node.key }
                }
                return data.result;
            },
            activate: function (event, data) {
                var count = (data.node.key.match(/\//g) || []).length;
                if (count > 3) {
                    currentFolderPath = data.node.key;

                    var imageSize = driveManager.getImageSizeByViewMode(viewMode);
                    $(".folder-name").text(data.node.title);
                    driveManager.showFolderImages(data.node.key, imageSize);
                }
                else {
    ...
    generateHtmlForGalleryView: async function (images) {
        ...
        for (var i = 0; i < images.length; i++) {
            ...
            if (imageData.rawImagePath.toLowerCase().includes("tif") == true || imageData.rawImagePath.toLowerCase().includes("tiff") == true) {
                imageData.rawImagePath = imageData.rawImagePath.replace("dprp12nc61odv.cloudfront.net", "dprp12nc61odv.cloudfront.net/filters:autojpg()");
            }

            if (imageData.fullPath.toLowerCase().includes("tif") == true || imageData.fullPath.toLowerCase().includes("tiff") == true) {
                imageData.fullPath = imageData.fullPath.replace("dprp12nc61odv.cloudfront.net", "dprp12nc61odv.cloudfront.net/filters:autojpg()");
            }

            if (imageData.rawImagePath.toLowerCase().includes("png") == false) {
                imageData.rawImagePath = imageData.rawImagePath.replace("filters:background_color(gainsboro)/", "");
            }

            if (imageData.fullPath.toLowerCase().includes("png") == false) {
                imageData.fullPath = imageData.fullPath.replace("filters:background_color(gainsboro)/", "");
            }
