        ...
        public async Task<IActionResult> SaveMarkup([FromBody] RejectFileRequestModel model)
        {
            ...
            using (var ftp = await _fluentFtpService.CreateFtpClient(fileServerViewModel))
            {
                ...
                try
                {
                    ...
                    // Check if the item was previously rejected before proceeding

                    var checkExistFeedback = await _feedbackReworkService.CheckExistingFeedback(feedbackOrderItem);

                    if (checkExistFeedback.Result == null)
                    {
