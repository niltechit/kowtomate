...
namespace KowToMateAdmin.Services
{
    public class DynamicReportPageViewFilterService : IDynamicReportPageViewFilterService
    {
            ...
            )
        {
            ...
            var templateTableCoumns = new List<GridViewSetupColumnSlimModel>();
            ...
            if (dynamicReportPagePropertyVM.selectedGridViewSetup.Id == -1)
            {
                ...
                templateTableCoumns = dynamicReportPagePropertyVM.tableColumns.Select(f => new GridViewSetupColumnSlimModel { ColoumnId = f.Id, Width = Convert.ToDouble(f.Width), DisplayOrder = (int)f.DisplayOrder }).ToList();
            }
            else
            {
                ...
                if (templateTableCoumns != null)
                {
                    ...
                    var notInTemplateTableCoumns = dynamicReportPagePropertyVM.tableColumns.Where(f => !templateTableCoumns.Any(i => i.ColoumnId == f.Id))
                        .Select(f => new GridViewSetupColumnSlimModel { ColoumnId = f.Id, Width = Convert.ToDouble(f.Width), DisplayOrder = (int)f.DisplayOrder }).ToList();
