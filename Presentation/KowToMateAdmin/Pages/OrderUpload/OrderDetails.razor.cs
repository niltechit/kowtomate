    ...
    public partial class OrderDetails
    {
        ...
        private async Task LoadOrderItemForLoginUser(bool isReloadOrder = true)
        {
            ...
            else
            {
                ...
                //if(order.InternalOrderStatus < (int)InternalOrderStatus.ReadyToDeliver)
                //{
                //    UpdateDuplicateUploadFileFromEditor();
                //}

                //FilterDuplicateUploadFile();
        ...
        private async void UpdateDuplicateUploadFileFromEditor()
        {
           List<ClientOrderItemModel>  orderItemModels = clientOrderItems
                                .Where(item => item.ProductionDoneFilePath != null && item.FileGroup == (int)OrderItemFileGroup.Work 
                                && item.Status > (int)InternalOrderItemStatus.InProduction && item.Status < (int)InternalOrderItemStatus.ReadyToDeliver
                                )
                                .GroupBy(item => new { item.ProductionDoneFilePath })
                                .Select(group => group.OrderBy(item => item.Id).First())
                                .ToList();

            if (orderItemModels.Any())
            {
                foreach (ClientOrderItemModel item in orderItemModels)
                {
                    item.FileGroup = (int)OrderItemFileGroup.Duplicate;
                    await _clientOrderItemService.UpdateClientOrderItemStatus(item);
                }
                await UpdateOrder(order, await GetInternalOrderStatus((int)order.Id));
            }
            
        }
        private async void FilterDuplicateUploadFile()
        {
            var duplicateAfterFilterQcUploadList = clientOrderItems
                       .Where(item => item.InternalFileOutputPath != null)
                       .GroupBy(item => new { item.InternalFileOutputPath })
                       .Select(group => group.First())
                       .ToList();

            var nonDuplicateList = clientOrderItems
                       .Where(item => item.InternalFileOutputPath == null && item.Status < (int)InternalOrderItemStatus.ReadyToDeliver)
                       .ToList();

            clientOrderItems = duplicateAfterFilterQcUploadList.Concat(nonDuplicateList).ToList();


            clientOrderItems = clientOrderItems
                       .Where(item => item.FileGroup != (int)OrderItemFileGroup.Duplicate)
                       .ToList();
        }
        ...
        private async Task PopulateMissingRawFiles()
        {
            ...
            {
                var matchedItem = orderAllItem.FirstOrDefault(x => Path.GetFileNameWithoutExtension(x.InternalFileInputPath) == Path.GetFileNameWithoutExtension(item.InternalFileOutputPath));
                if (matchedItem != null)
                {
                    ...
                    var downloadTasks = selectedFiles.Select(async orderItem =>
                    {
                        ...
                        try
                        {
                            using (var client = _ftpSharpLibraryService.CreateAsyncFtpClient(model))
                            {
                                ...
                                dataSavePath = dataSavePath.TrimEnd();
                                var localPath = $"{dataSavePath}/{model.fileName}";
                                var remotePath = $"{model.UploadDirectory}/{model.fileName}";
            ...
            foreach (var item in items)
            {
                ...
                if (endPath.Contains("/"))
                {
                    ...
                    if (orderItemStatusWiseViewMode == 1)
                    {
                        ...
                        GetRawFileContainerFolderFileCount(items, endPath, folderNode, imageExtensions);
                    }
                    if (orderItemStatusWiseViewMode == 2)
                    {
                        ...
                        GetProductionDoneFileContainerFolderFileCount(items, endPath, folderNode, imageExtensions);
                    }
                    if (orderItemStatusWiseViewMode == 3)
                    {
                        ...
                        GetCompletedFileContainerFolderFileCount(items, endPath, folderNode, imageExtensions);
                    }
        ...
        private static void GetRawFileContainerFolderFileCount(List<ClientOrderItemModel> items, string endPath, FolderNodeModel folderNode, HashSet<string> imageExtensions)
        {
            if (endPath.Split('/').Length == 2 && imageExtensions.Contains(Path.GetExtension(endPath.Split('/')[1])))
            {
                folderNode.ParentFolderContainFileCount = items.Where(i => i.InternalFileInputPath
                                                    .Contains(endPath.Split('/')[0])).Count();
            }
        }

        private static void GetProductionDoneFileContainerFolderFileCount(List<ClientOrderItemModel> items, string endPath, FolderNodeModel folderNode, HashSet<string> imageExtensions)
        {
            if (endPath.Split('/').Length == 2 && imageExtensions.Contains(Path.GetExtension(endPath.Split('/')[1])))
            {
                folderNode.ParentFolderContainFileCount = items.Where(i => i.ProductionDoneFilePath
                                                    .Contains(endPath.Split('/')[0])).Count();
            }
        }

        private static void GetCompletedFileContainerFolderFileCount(List<ClientOrderItemModel> items, string endPath, FolderNodeModel folderNode, HashSet<string> imageExtensions)
        {
            if (endPath.Split('/').Length == 2 && imageExtensions.Contains(Path.GetExtension(endPath.Split('/')[1])))
            {
                folderNode.ParentFolderContainFileCount = items.Where(i => i.InternalFileOutputPath
                                                    .Contains(endPath.Split('/')[0])).Count();
            }
        }
        ...
        private async Task RejectFromPopupView(InternalOrderItemStatus status = InternalOrderItemStatus.InQc, int imageId = 0)
        {
            ...
            if (imageId > 0)
            {
                await js.ShowToast(SweetAlertTypeMessagee.success, "Operation completed successfully!", "Success");
                var serverInfo = await _fileServerService.GetById((int)order.FileServerId);
                var contactInfo = await _contactManager.GetById(loginUser.ContactId);
